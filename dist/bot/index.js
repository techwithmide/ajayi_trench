...
import { createPosition, getPositionByIdForChat, getOpenPositionByMintForChat, getOpenPositionsForChat, getClosedPositionsForChat, getLeaderboardGlobal, getCachedTelegramUser, upsertTelegramUser, closePosition, } from "../db/index.js";
...
async function resolveLeaderboardLabel(api, e) {
...
export function createBot(token) {
    ...
    bot.use(async (ctx, next) => {
        try {
            ...
            if (typeof text === "string") {
                console.log(`[Update] chat=${ctx.chat?.id ?? "?"} type=${ctx.chat?.type ?? "?"} text=${text}`);
            }
        }
        catch {
    ...
    bot.command("start", async (ctx) => {
        ...
        await ctx.reply(`<b>AJAYI Trench Bot</b>\n\n` +
            ...
            `/close &lt;id&gt; — Manually close a position`, { parse_mode: "HTML" });
    });
    ...
    bot.on("message:text", async (ctx, next) => {
        ...
        if (existing) {
            await ctx.reply(`⚠️ Already tracking <b>${existing.symbol}</b>\n` +
                `<code>${truncateAddress(mint)}</code> | ID: <code>${existing.id}</code>`, { parse_mode: "HTML" });
            return;
        }
        const status = await ctx.reply("🔍 Fetching token info…");
        const edit = (text) => ctx.api.editMessageText(chatId, status.message_id, text, {
            parse_mode: "HTML",
        });
        try {
            ...
            if (!tokenInfo) {
                await edit(`❌ Token not found on DexScreener or GeckoTerminal.\n\n` +
            ...
            await edit(`<b>Position Opened — $${tokenInfo.symbol}</b>\n\n` +
    ...
    bot.command("positions", async (ctx) => {
        ...
        if (positions.length === 0) {
            await ctx.reply("No open positions. Paste a Solana token address to start tracking.");
            return;
        }
        ...
        await ctx.reply(`<b>Open Positions (${positions.length})</b>\n\n${lines.join("\n\n")}`, { parse_mode: "HTML" });
    });
    // ─── /history ──────────────────────────────────────────────────────────────
    bot.command("history", async (ctx) => {
        ...
        const lines = recent.map((p, i) => {
            ...
            const emoji = isWin ? "🟢" : "🔴";
            const tag = p.exitReason === "TAKE_PROFIT"
                ? "TP"
        ...
        await ctx.reply(`<b>Trade History</b> (last ${recent.length} of ${positions.length})\n\n${lines.join("\n\n")}`, { parse_mode: "HTML" });
    });
    // ─── /pnl ──────────────────────────────────────────────────────────────────
    bot.command("pnl", async (ctx) => {
        ...
        await ctx.reply(`<b>Performance Summary</b>\n\n` +
            `<b>Trades</b>\n` +
            ...
            `Active watchers (your chat): <b>${getActiveWatcherCountForChat(chatIdStr)}</b>`, { parse_mode: "HTML" });
    });
    // ─── /leaderboard ─────────────────────────────────────────────────────────
    bot.command("leaderboard", async (ctx) => {
        const rows = getLeaderboardGlobal();
        if (rows.length === 0) {
            await ctx.reply("<b>Global leaderboard</b>\n\nNo closed trades yet. " +
                "When any user’s positions hit TP/SL or are /closed, stats appear here.", { parse_mode: "HTML" });
            return;
        }
        const medals = ["🥇", "🥈", "🥉"];
        ...
        const lines = await Promise.all(top.map(async (e, i) => {
            ...
            const wr = e.trades > 0 ? ((e.wins / e.trades) * 100).toFixed(1) : "0.0";
            return [
                `${medal}<b>${who}</b>`,
        ...
        const more = rows.length > top.length
            ...
            : "";
        await ctx.reply(`<b>Global leaderboard</b> — all chats (realised P&amp;L, closed trades only)\n\n` +
            lines.join("\n\n") +
    ...
    bot.command("close", async (ctx) => {
        ...
        if (!position || position.status !== "OPEN") {
            await ctx.reply(`❌ Position <code>${posId}</code> not found, already closed, or not yours in this chat.`, { parse_mode: "HTML" });
            return;
        }
        const closing = await ctx.reply(`⏳ Fetching exit price for <b>$${position.symbol}</b>…`, { parse_mode: "HTML" });
        ...
        const exitUsd = trade?.outputUsd ?? (currentPrice / position.entryPrice) * VIRTUAL_USD;
        stopWatcher(posId);
        const closed = closePosition(posId, currentPrice, exitUsd, "MANUAL");
        if (!closed) {
            await ctx.api.editMessageText(ctx.chat.id, closing.message_id, "❌ Failed to close position.");
            return;
        }
        const jupBlock = trade
            ? `Route: ${trade.route}\n📉 Exit slippage: ${trade.priceImpactPct.toFixed(2)}%`
            : `Jupiter unavailable — P&L estimated from price`;
        await ctx.api.editMessageText(ctx.chat.id, closing.message_id, `<b>Manually Closed — $${position.symbol}</b>\n\n` +
            ...
            `${jupBlock}\n` +
            `⏱ Held: ${formatDuration(position.entryTime, Date.now())}`, { parse_mode: "HTML" });
    });
    return bot;
}
