...
export function stopWatcher(positionId) {
...
export function getActiveWatcherCountForChat(chatId) {
    const open = getOpenPositionsForChat(chatId);
    return open.filter((p) => activeWatchers.has(p.id)).length;
}
export async function restoreWatchers(notify) {
...
async function tick(position, notify) {
    const currentPrice = await fetchCurrentPrice(position.mint);
    if (!currentPrice) {
        console.warn(`[Watcher] No price for ${position.symbol} - will retry next tick`);
        return;
    }
...
async function exit(position, currentPrice, reason, notify) {
